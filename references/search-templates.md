...
### 9-A. ⚠️ 네이버 브랜드몰(`brand.naver.com`) 크롤링 차단 주의
...
### 10-2A. 제품명만 입력 시 성분표 Fallback 파이프라인
...
| **brand.naver.com WebFetch 차단** | 제조사 공식 홈페이지 | 식약처 건기식 DB | "네이버 브랜드몰 차단, 대체 소스 사용" |
| **공식몰 "상품상세 참조"만 있음** | 쿠팡/11번가 상세 | 식약처 건기식 DB | "공식몰 성분 미표기, 2차 소스로 확인" |
| **Step A~D 모두 실패** | 사용자 재질문 | — | "공개 성분 정보 없음, 사용자 입력 필요" |
